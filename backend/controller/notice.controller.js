...
export const deleteNotice = async (req, res) => {
  try {
    ...
    // Archive the notice before deletion
    try {
      await archiveItem('notice', notice._id, req.user.id, 'Notice deleted by user');
      console.log(`Notice "${notice.title}" archived successfully`);
    } catch (archiveError) {
      console.error("Error archiving notice:", archiveError);
