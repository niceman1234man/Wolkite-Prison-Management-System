...
const ViewInstruction = () => {
  ...
  const deleteInstruct = async () => {
    try {
      const confirm = window.confirm("Are you sure you want to delete this Instruction ?");
      if (confirm) {
        const deletedInstruct = await axiosInstance.delete(`/instruction/delete/${id}`);
        if (deletedInstruct) {
...
console.log(instruction)
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md">
      ...
      <h2 className="text-2xl font-bold mb-6 text-center">Instruction Details</h2>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-lg font-medium">{instruction.courtCaseNumber}</p>
            </div>
            ...
            <div>
              ...
              <div>
                ...
                <img src={`https://localhost:4000/uploads/${instruction.signature}`} alt="Attachment" />
              </div>
          ...
          <button
              className="bg-blue-600 text-white py-2 px-3 rounded font-semibold w-[70px]"
